...
#define COUT_FOR_NODE if (-1 != GetNode()->GetId())\
	 std::cout
...
void 
BitcoinNode::SetDagViews (std::string *dagViews)
{
  NS_LOG_FUNCTION (this);
...
BitcoinNode::StopApplication ()     // Called at time specified by Stop
{
  ...
  std::cout<<"Node[ "<<GetNode()->GetId()<<" ] Total Blocks = " << m_blockchain.GetTotalBlocks()<<"\n";
...
BitcoinNode::ReceivedBlockMessage(std::string &blockInfo, Address &from) 
{
  ...
  {  
    ...
    {*/
      ...
      newBlock.m_witnessLevel = d["blocks"][j]["witnessedLevel"].GetInt();
